    ...
    return (
        <main className="min-h-[calc(100dvh-65px)] bg-gray-100 md:p-8 p-4">
            ...
            <div className="grid grid-cols-[7fr_3fr] gap-4">
                ...
                <div>
                    <h2 className="text-xl font-semibold text-slate-950 mb-2">Vos missions</h2>
                    {tasks.map(task => (
                        <div key={task.id} className="bg-white border border-slate-200 rounded-lg p-4 mb-4">
                            <div className="flex justify-between mb-2">
                                <h3 className="font-semibold">{task.title}
                                    <span className={`${task.status === "done" ? "bg-green-200 text-green-800" : "bg-red-200 text-orange-800"} inline-flex items-center gap-1.5 h-6 text-xs text-emerald-400 bg-emerald-950 hover:bg-primary/80 font-semibold px-2 rounded-md border border-transparent transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 ml-2`}>
                                        {task.status === "done" ? "Terminée" : "En cours"}
                                    </span>
                                </h3>
                                <span className="text-xs text-slate-500">{task.dateStart} - {task.dateEnd}</span>
                            </div>
